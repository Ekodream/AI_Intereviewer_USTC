...
def extract_text_from_pdf(pdf_path: str) -> str:
    ...
    try:
        with pdfplumber.open(pdf_path) as pdf:
            print(f"📄 [PDF解析] 共 {len(pdf.pages)} 页")
            for i, page in enumerate(pdf.pages):
                page_text = page.extract_text()
                print(f"📄 [PDF解析] 第{i+1}页提取: {len(page_text) if page_text else 0} 字符")
                if page_text:
                    text += page_text + "\n"
    except Exception as e:
        raise Exception(f"PDF 解析失败：{str(e)}")
    
    print(f"📄 [PDF解析] 总计提取: {len(text)} 字符")
    print(f"📄 [PDF解析] 前500字符预览: {text[:500]}")
    return text.strip()
...
def analyze_resume_with_llm(resume_text: str) -> Dict[str, Any]:
    ...
    Returns:
        结构化分析结果（字典）
    """
    print(f"🤖 [LLM分析] 输入文本长度: {len(resume_text)} 字符")
    ...
    try:
        ...
        print(f"🤖 [LLM分析] 原始响应长度: {len(response_text)} 字符")
        print(f"🤖 [LLM分析] 原始响应预览: {response_text[:500]}")
        ...
        print(f"🤖 [LLM分析] 解析成功，字段: {list(analysis_result.keys())}")
