...
> **一款面向顶尖高校实验室申请场景的 AI 面试系统**，通过大语言模型模拟不同面试风格，提供全流程、多维度、沉浸式的面试训练体验。前端采用 **「指挥舱」** 深空主题 Dashboard 设计，所有核心功能一屏可达，零 Tab 切换。
...
系统支持完整的 **9 阶段面试流程**，顶部进度条实时追踪当前阶段：

```
开始 → 自我介绍 → 经历深挖 → 基础知识 → 科研动机 → 科研潜力 → 综合追问 → 学生反问 → 结束
```
...
- **IDE 手动控制** — 可按需开启/关闭编辑器面板，支持代码试验与提交点评
...
- **温和型** — 循循善诱，给予充分提示
- **正常型** — 导师面试常规节奏，平衡考察基础与潜力
- **压力型** — 高频深挖，检验科研严谨性与抗压表达

### 提示词与流程约束

当前预设提示词保持 **温和型 / 正常型 / 压力型** 三种风格，但统一采用 9 阶段流程与严格推进规则：

- 阶段编号固定为 `0..8`：开始、自我介绍、经历深挖、基础知识、科研动机、科研潜力、综合追问、学生反问、结束。
- 仅当当前阶段完成时输出推进口令：`我们进入面试的下一个环节 /next[(下一个环节编号)]`。
- 大模型输出中的控制口令与控制短语仅用于内部状态推进，不会在页面文本与 TTS 播报中展示。
- 提示词已移除对时长的硬性约束，重点强调结构化推进与问题质量。
